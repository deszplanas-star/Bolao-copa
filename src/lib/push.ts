...
/**
 * Envia push só para os user_ids listados (ex: quem cravou — BROCOU!).
 */
export async function sendPushToUsers(userIds: string[], event: PushEvent): Promise<PushResult> {
  if (userIds.length === 0) return { ok: true, sent: 0, subs: 0, pruned: 0 };
  ...
  const payload = JSON.stringify({
    title: event.title,
    body: event.body,
    tag: event.tag ?? "bolao26-brocou",
    url: event.url ?? "/apostas?tab=ranking",
  });
